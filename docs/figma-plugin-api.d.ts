// Tell TypeScript that these typings are used in a context where the DOM is not available,
// but ES6 APIs (like Array) are.  This helps us avoid accidentally referring to a DOM type.
...
interface PluginAPI {
     ...
     * [Example: how to work with images](/docs/plugins/working-with-images).
     * @remarks
     *
     ...
     *
     * @throws needAtLeastOne, insertIndexTooLarge, groupSamePage, cannotMoveRoot, cannotMoveCycle, cannotMoveNonPageToRoot, cannotMovePageToNonRoot, cannotMoveNoChildren, cannotMoveInternalParent, cannotMoveInternal, cannotMoveToInstance, cannotMoveInInstance, cannotMoveComponentCycle, cannotMoveNestedComponent, cannotMoveComponentSetCycle, cannotMoveNonComponentToComponentSet, cannotMoveToPageDivider, cannotMoveToSlot
     */
...
/**
 * @pageId figma-viewport
 */
interface ViewportAPI {
}
/**
 * @pageId figma-parameters
 */
...
interface ExportSettingsSVGBase {
    ...
    readonly colorProfile?: 'DOCUMENT' | 'SRGB' | 'DISPLAY_P3_V4'; // defaults to 'DOCUMENT'
}
/**
 * @pageId ExportSettings
 */
...
interface ExportSettingsREST {
     ...
     * ```
     *
     * For more information on the shape of the output of the 'JSON_REST_V1' format, see the [files](/docs/rest-api/files) documentation.
     */
...
/**
 * @pageId properties/VectorPath-windingrule
 */
type WindingRule = 'NONZERO' | 'EVENODD';
/**
 * @pageId VectorNetwork
 */
...
interface BaseNodeMixin extends PluginDataMixin, DevResourcesMixin {
     ...
     * @param command - The string that will be passed as `figma.command` when the plugin is run after the button is clicked. This command must be present in the [manifest](/docs/plugins/manifest#relaunchbuttons) under one of the `relaunchButtons`, which is used to look up the name to display for the button.
     ...
     * ![Relaunch UI in Figma Design](/img/plugins/relaunch_ui_design.png)
     *
     * ### Example FigJam UI
     * ![Relaunch UI in FigJam](/img/plugins/relaunch_ui_figjam.png)
     */
...
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/rc/variables.ts:variableBindableNodeFields)
// KEEP_IN_SYNC(fullscreen/lib/scenegraph/design-systems/FGVariablesApiHelpers.cpp:pluginStringToSimpleVariableField)
...
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/src/variables.ts:variableBindableTextFields)
...
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/src/variables.ts:variableBindablePaintFields)
type VariableBindablePaintField = 'color';
type VariableBindablePaintStyleField = 'paints';
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/src/variables.ts:variableBindableColorStopFields)
type VariableBindableColorStopField = 'color';
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/src/variables.ts:variableBindableShadowEffectFields)
type VariableBindableEffectField = 'color' | 'radius' | 'spread' | 'offsetX' | 'offsetY';
type VariableBindableEffectStyleField = 'effects';
// KEEP_IN_SYNC(fullscreen/ts/scenegraph/src/variables.ts:variableBindableGridLayoutFields)
type VariableBindableLayoutGridField = 'sectionSize' | 'count' | 'offset' | 'gutterSize';
type VariableBindableGridStyleField = 'layoutGrids';
// This should be in sync with fullscreen/ts/scenegraph/src/variables.ts:variableBindableComponentPropertyFields
type VariableBindableComponentPropertyField = 'value';
// This should be in sync with fullscreen/ts/scenegraph/src/variables.ts:variableBindableComponentPropertyDefinitionFields
...
interface ChildrenMixin {
     ...
     * :::info
     *
     * If you are curious, the reason why inserting children has to be done via API calls is because our internal representation for the layer tree uses [fractional indexing](https://www.figma.com/blog/multiplayer-editing-in-figma/) and {@link ChildrenMixin.insertChild} performs that conversion.
     *
     * :::
     */
     ...
     *
     * @throws cannotMoveRoot, cannotMoveCycle, cannotMoveNonPageToRoot, cannotMovePageToNonRoot, cannotMoveNoChildren, cannotMoveInternalParent, cannotMoveInternal, cannotMoveToInstance, cannotMoveInInstance, cannotMoveComponentCycle, cannotMoveNestedComponent, cannotMoveComponentSetCycle, cannotMoveNonComponentToComponentSet, cannotMoveToPageDivider, cannotMoveToSlot
     */
     ...
     * If this is called on an auto-layout frame, calling this function can cause this layer to be resized and children to be moved.
     *
     * @throws insertIndexTooLarge, cannotMoveRoot, cannotMoveCycle, cannotMoveNonPageToRoot, cannotMovePageToNonRoot, cannotMoveNoChildren, cannotMoveInternalParent, cannotMoveInternal, cannotMoveToInstance, cannotMoveInInstance, cannotMoveComponentCycle, cannotMoveNestedComponent, cannotMoveComponentSetCycle, cannotMoveNonComponentToComponentSet, cannotMoveToPageDivider, cannotMoveToSlot
     */
     ...
     * Please refer to our [recommendations](/docs/plugins/accessing-document#optimizing-traversals) for how to optimize document traversals.
     *
     * :::
     */
     ...
     * ```
     */
    findAllWithCriteria<T extends NodeType[]>(criteria: FindAllCriteria<T>): Array<{
        type: T[number];
    } & SceneNode>;
    /**
...
interface DimensionAndPositionMixin {
     ...
     * :::info
     *
     * Why this complication? We do it this way because groups and boolean operations automatically resize to fit their children. While you _can_ set the relative transform of a group to move it, it's a property derived from the position and size of its children.
...
interface BlendMixin extends MinimalBlendMixin {
    ...
    maskType: MaskType;
    /**
     * Array of effects. See {@link Effect} type. For help on how to change this value, see [Editing Properties](/docs/plugins/editing-properties).
     */
...
declare type StrokeCap = 'NONE' | 'ROUND' | 'SQUARE' | 'ARROW_LINES' | 'ARROW_EQUILATERAL' | 'DIAMOND_FILLED' | 'TRIANGLE_FILLED' | 'CIRCLE_FILLED';
 ...
 * @pageId GridTrackReorderOptions
 */
...
interface GridLayoutMixin {
     ...
     * :::caution
     *
     * ⚠️ Setting `gridRowCount` will throw an error if {@link GridLayoutMixin.gridAutoTracks} is set to `"ROWS"`, since the row count is managed automatically.
...
interface GridChildrenMixin {
     ...
     * :::caution
     *
     * ⚠️ This method will throw an error if the parent grid has {@link GridLayoutMixin.gridItemsPositioning} set to `"ROW_AUTO_FLOW"`, since child positions are managed automatically. Use {@link ChildrenMixin.insertChild} to change the order of children instead.
     *
     * :::
...
/**
 * @pageId InferredAutoLayoutResult
 */
...
interface MinimalStrokesMixin {
     ...
     * :::caution
     *
     * For rectangle nodes or frame-like nodes using different individual stroke weights, this property will return {@link PluginAPI.mixed}.
     *
     * :::
     *
     * :::info
...
/**
 * @pageId VariableWidthStrokeProperties
 */
 ...
 * @pageId VariableWidthStrokeProperties
 */
declare type VariableWidthStrokeProperties = PresetVariableWidthStrokeProperties | CustomVariableWidthStrokeProperties;
/**
 * @pageId ComplexStrokeProperties
 */
declare type ComplexStrokeProperties = {
    type: 'BASIC';
} | DynamicStrokeProperties | BrushStrokeProperties;
/**
 * @pageId ComplexStrokeProperties
 */
interface ScatterBrushProperties {
    ...
    brushType: 'SCATTER';
    /**
     * Name of the scatter brush. See the [available brushes](/api/complex-stroke-properties.md#available-brushes) for previews of these brushes.
     * Nodes using custom brushes will have this set to 'CUSTOM'. However, setting this property to 'CUSTOM' is not yet supported.
     */
    brushName: 'BUBBLEGUM' | 'WITCH_HOUSE' | 'SHOEGAZE' | 'HONKY_TONK' | 'SCREAMO' | 'DRONE' | 'DOO_WOP' | 'SPOKEN_WORD' | 'VAPORWAVE' | 'OI' | 'CUSTOM';
...
interface StretchBrushProperties {
    ...
    brushType: 'STRETCH';
    /**
     * Name of the stretch brush. See the [available brushes](/api/complex-stroke-properties.md#available-brushes) for previews of these brushes.
     * Nodes using custom brushes will have this set to 'CUSTOM'. However, setting this property to 'CUSTOM' is not yet supported.
     * */
    brushName: 'HEIST' | 'BLOCKBUSTER' | 'GRINDHOUSE' | 'BIOPIC' | 'SPAGHETTI_WESTERN' | 'SLASHER' | 'HARDBOILED' | 'VERITE' | 'EPIC' | 'SCREWBALL' | 'ROM_COM' | 'NOIR' | 'PROPAGANDA' | 'MELODRAMA' | 'NEW_WAVE' | 'CUSTOM';
 ...
 * @pageId node-properties
 */
interface BaseFrameMixin extends BaseNodeMixin, SceneNodeMixin, ChildrenMixin, ContainerMixin, DeprecatedBackgroundMixin, GeometryMixin, ComplexStrokesMixin, CornerMixin, RectangleCornerMixin, BlendMixin, ConstraintMixin, LayoutMixin, ExportMixin, IndividualStrokesMixin, AutoLayoutMixin, GridLayoutMixin, AspectRatioLockMixin, AnnotationsMixin, DevStatusMixin {
    /**
    ...
    /**
     * Array of {@link LayoutGrid} objects used as layout grids on this node. For help on how to change this value, see [Editing Properties](/docs/plugins/editing-properties).
     */
    ...
    /**
     * Array of {@link Guide} used inside the frame. Note that each frame has its own guides, separate from the canvas-wide guides. For help on how to change this value, see [Editing Properties](/docs/plugins/editing-properties).
     */
...
/**
 * @pageId MeasurementSide
 */
type MeasurementSide = 'TOP' | 'RIGHT' | 'BOTTOM' | 'LEFT';
/**
 * @pageId MeasurementOffset
 */
...
interface MeasurementsMixin {
     ...
     * :::
     */
    editMeasurement(id: string, newValue: {
        offset?: MeasurementOffset;
        freeText?: string;
    }): Measurement;
    /**
...
interface ComponentPropertiesMixin {
     ...
     *
     * @throws cannotChangeDefaultValueOfSlotProperty
     */
    editComponentProperty(propertyName: string, 
    // prettier-ignore
    newValue: {
        name?: string;
...
interface BaseNonResizableTextMixin {
     ...
     */
    getRangeOpenTypeFeatures(start: number, end: number): {
        readonly [feature in OpenTypeFeature]: boolean;
    } | PluginAPI['mixed'];
    /**
     ...
     */
    getRangeBoundVariable(start: number, end: number, field: VariableBindableTextField): VariableAlias | null | PluginAPI['mixed'];
    /**
     ...
     * @remarks
     *
     * This function provides an easy and performant way to get multiple text properties which may have [mixed values](/docs/plugins/working-with-text#mixed-styles), along with which characters these values apply to.
...
interface NonResizableTextMixin extends BaseNonResizableTextMixin {
     ...
     */
    getRangeTextDecorationStyle(start: number, end: number): TextDecorationStyle | PluginAPI['mixed'] | null;
    /**
     ...
     */
    getRangeTextDecorationOffset(start: number, end: number): TextDecorationOffset | PluginAPI['mixed'] | null;
    /**
     ...
     */
    getRangeTextDecorationThickness(start: number, end: number): TextDecorationThickness | PluginAPI['mixed'] | null;
    /**
     ...
     */
    getRangeTextDecorationColor(start: number, end: number): TextDecorationColor | PluginAPI['mixed'] | null;
    /**
...
interface PageNode extends BaseNodeMixin, ChildrenMixin, ExportMixin, ExplicitVariableModesMixin, MeasurementsMixin {
    /**
...
interface GroupNode extends BaseNodeMixin, SceneNodeMixin, ReactionMixin, ChildrenMixin, ContainerMixin, DeprecatedBackgroundMixin, BlendMixin, LayoutMixin, ExportMixin, AspectRatioLockMixin {
    /**
...
/**
 * @pageId TransformGroupNode
 */
interface TransformGroupNode extends BaseNodeMixin, SceneNodeMixin, ReactionMixin, ChildrenMixin, ContainerMixin, DeprecatedBackgroundMixin, BlendMixin, LayoutMixin, ExportMixin, AspectRatioLockMixin {
    /**
...
interface RectangleNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, ComplexStrokesMixin, RectangleCornerMixin, IndividualStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
...
interface LineNode extends DefaultShapeMixin, ConstraintMixin, AnnotationsMixin, ComplexStrokesMixin {
    /**
...
interface EllipseNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, ComplexStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
...
interface PolygonNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, ComplexStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
...
interface StarNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, ComplexStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
...
interface VectorNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, ComplexStrokesMixin, VectorLikeMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
...
interface TextNode extends DefaultShapeMixin, ConstraintMixin, NonResizableTextMixin, ComplexStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
 ...
 * @pageId TextPathNode
 */
interface TextPathNode extends DefaultShapeMixin, ConstraintMixin, NonResizableTextPathMixin, ComplexStrokesMixin, AnnotationsMixin, AspectRatioLockMixin {
    /**
     ...
     */
    clone(): TextPathNode; // cloned node starts off inserted into current page
    /**
     * The horizontal alignment of the text with respect to the textbox. Setting this property requires the font the be loaded.
     */
    textAlignHorizontal: 'LEFT' | 'CENTER' | 'RIGHT' | 'JUSTIFIED';
...
/**
 * @pageId ComponentPropertyOptions
 */
...
/**
 * @pageId ComponentPropertyDefinitions
 */
...
interface ComponentNode extends DefaultFrameMixin, PublishableMixin, VariantMixin, ComponentPropertiesMixin {
     ...
     *
     * @throws cannotCreateSlotInReadOnly
     */
    createSlot(): SlotNode;
    /**
...
interface BooleanOperationNode extends DefaultShapeMixin, ChildrenMixin, CornerMixin, ComplexStrokesMixin, ContainerMixin, AspectRatioLockMixin {
    /**
...
interface StampNode extends DefaultShapeMixin, ConstraintMixin, StickableMixin, AspectRatioLockMixin {
    /**
...
interface HighlightNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, VectorLikeMixin, StickableMixin, AspectRatioLockMixin {
    /**
...
type VariableScope = 'ALL_SCOPES' | 'TEXT_CONTENT' | 'CORNER_RADIUS' | 'WIDTH_HEIGHT' | 'GAP' | 'ALL_FILLS' | 'FRAME_FILL' | 'SHAPE_FILL' | 'TEXT_FILL' | 'STROKE_COLOR' | 'STROKE_FLOAT' | 'EFFECT_FLOAT' | 'EFFECT_COLOR' | 'OPACITY' | 'FONT_FAMILY' | 'FONT_STYLE' | 'FONT_WEIGHT' | 'FONT_SIZE' | 'LINE_HEIGHT' | 'LETTER_SPACING' | 'PARAGRAPH_SPACING' | 'PARAGRAPH_INDENT';
/**
 * @pageId VariableCodeSyntaxPlatform
 */
 ...
 * @pageId SectionNode
 */
interface SectionNode extends ChildrenMixin, MinimalFillsMixin, OpaqueNodeMixin, DevStatusMixin, AspectRatioLockMixin, MinimalStrokesMixin, CornerMixin, RectangleCornerMixin {
    /**
...
/**
 * @pageId InteractiveSlideElementNode
 */
 ...
 * @pageId node-types
 */
type SceneNode = SliceNode | FrameNode | GroupNode | ComponentSetNode | ComponentNode | InstanceNode | BooleanOperationNode | VectorNode | StarNode | LineNode | EllipseNode | PolygonNode | RectangleNode | TextNode | TextPathNode | TransformGroupNode | StickyNode | ConnectorNode | ShapeWithTextNode | CodeBlockNode | StampNode | WidgetNode | EmbedNode | LinkUnfurlNode | MediaNode | SectionNode | HighlightNode | WashiTapeNode | TableNode | SlideNode | SlideRowNode | SlideGridNode | SlotNode | InteractiveSlideElementNode;
/**
 * @pageId node-types
 */
...
interface AiAPI {
    /**
     * Generates a new image from a natural-language prompt (text-to-image).
     *
     * Returns PNG bytes; pass them to `figma.createImage(bytes)` and attach
     * the resulting `imageHash` to a node's fills.
     *
     * @param prompt - Natural-language description of the desired image.
     * @param size - Target pixel dimensions for the output.
     * @returns PNG bytes of the generated image, which can be passed to `figma.createImage(bytes)`.
     *
     * ```ts title="Generate an image and place it as a fill"
     * const bytes = await figma.ai.createImageAsync("a sleeping cat on a windowsill", { x: 512, y: 512 })
     * const image = figma.createImage(bytes)
     * const rect = figma.createRectangle()
     * rect.resize(512, 512)
     ...
     * ```
     */
    createImageAsync(prompt: string, size: Vector): Promise<Uint8Array>;
    /**
     * Edits or composes one or more existing image fills using a
     * natural-language prompt (image-to-image).
     *
     * Pass one or more hashes (from existing image fills in the scene, read
     * via `(paint as ImagePaint).imageHash`). `imageHashes` must be
     * non-empty — use `createImageAsync` for text-to-image generation
     * instead. With multiple hashes, the output adopts the aspect ratio of
     * the last image provided.
     *
     * @param imageHashes - Hashes of existing image fills to use as input. Must be non-empty.
     * @param prompt - Natural-language description of the desired edit or composition.
     * @param size - Target pixel dimensions for the output.
     * @returns PNG bytes of the edited image, which can be passed to `figma.createImage(bytes)`.
     *
     * ```ts title="Edit an existing image fill"
     * const isImagePaint = (p: Paint): p is ImagePaint => p.type === "IMAGE"
     * if (!("fills" in node) || !Array.isArray(node.fills)) return
     * const sourcePaint = node.fills.find(isImagePaint)
     * if (!sourcePaint || sourcePaint.imageHash === null) return
     * const sourceHash = sourcePaint.imageHash
     *
     * const bytes = await figma.ai.editImageAsync(
     *   [sourceHash],
     *   "add a knitted wizard hat on the subject's head",
     *   { x: 512, y: 512 },
     * )
     * const newImage = figma.createImage(bytes)
     *
     * if (node.removed || !Array.isArray(node.fills)) return
     * node.fills = node.fills.map((p) =>
     *   isImagePaint(p) && p.imageHash === sourceHash
     *     ? { ...p, imageHash: newImage.hash }
     *     : p,
     * )
     * ```
     */
    editImageAsync(imageHashes: string[], prompt: string, size: Vector): Promise<Uint8Array>;
}
