...
## Screenshot Script Template
    ...
    const ctx = await browser.newContext({
      viewport: { width: 1568, height: 900 },  // 900, NOT 4000 — see Cropping section
    ...
    // Dismiss overlays / country selectors (site-specific — see Known Sites)
    await page.evaluate(() => {
    ...
    // Inject fonts AFTER page load (not addInitScript — see Font Injection)
    await injectFonts(page);

    // Handle video heroes (see Video Heroes section)
    ...
    // Re-inject after scroll (dynamic content may replace elements)
    await injectFonts(page);
    await injectVideoFrames(page);
...
Take a `fullPage: true` screenshot then crop to 4000px with this pure-Python PNG cropper (no PIL required) — save as `/tmp/crop_png.py`:
